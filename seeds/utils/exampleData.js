...
export async function getExampleSpecializationTree(count = 2, subCount = 3) {
  ...
  let exampleSpecializations = getRandomElements((await specializationRepository.findMany({
    filter: undefined
  ...
  for (let specialization of exampleSpecializations) {
    const subSpecializationIds = getRandomElements((await specializationRepository.findSubSpecializations({ filter: {mainSpecializationId: specialization.id } })).subSpecializations, subCount).map(specialization => specialization.id);
...
export async function getExampleGovernmentIds(count = 3) {
  ...
  const randomSamples = getRandomElements(governments.governments, count).map(value => value.id);
  return randomSamples;
}

export async function getExampleCityIds(governmentId, count = 3) {
  const cities = (await governmentRepository.findCities({ filter: { id:governmentId } }));

  const randomSamples = getRandomElements(cities.cities, count).map(value => value.id);
  return randomSamples;
}
