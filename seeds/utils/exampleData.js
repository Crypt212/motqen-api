...
export async function getExampleSpecializationTree(count = 2, subCount = 3) {
  let exampleSpecializations = getRandomElements(
    (
      await specializationRepository.findMany({
        filter: undefined,
      })
    ).specializations,
    count
  ...
  for (let specialization of exampleSpecializations) {
    const subSpecializationIds = getRandomElements(
      (
        await specializationRepository.findSubSpecializations({
          filter: { mainSpecializationId: specialization.id },
        })
      ).subSpecializations,
      subCount
