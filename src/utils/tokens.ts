 ...
 */
function getTokenConfigFromPayload<T extends keyof TokenTypeMap>(payload: TokenTypeMap[T]): { secret: string, expiresIn: string } {
 ...
 */
export function verifyAndDecodeToken <T extends keyof TokenTypeMap> (token: string, expectedType: T): TokenTypeMap[T] {
 ...
 */
export function verifyHeaderToken <T extends keyof TokenTypeMap> (tokenString: string, type: T): TokenTypeMap[T] {
