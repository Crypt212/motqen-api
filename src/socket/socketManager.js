...
export async function initSocketServer(httpServer) {
  ...
  io.on('connection', async (socket) => {
    ...
    // 3. Notify all conversation partners that user is online
    try {
      ...
      for (const conv of convs) {
        ...
        if (partner && !partnersNotified.has(partner.userId)) {
          io.to(`user:${partner.userId}`).emit('user_online', { userId });
          partnersNotified.add(partner.userId);
        }
      }

      // 4. Emit missed_messages_available for each conversation with unread messages
