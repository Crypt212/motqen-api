...
export const deleteClientProfile = asyncHandler(async (req, res) => {
  ...
  const clientProfile = await clientService.delete({ clientProfileId: req.userState.client.id });
...
export const createWorkerProfile = asyncHandler(async (req, res) => {
  const { experienceYears, isInTeam, acceptsUrgentJobs, specializationsTree: specializationsTree, workGovernmentIds } = matchedData(req, { includeOptionals: true });
  const userId = req.userState.userId;
...
export const deleteWorkerProfile = asyncHandler(async (req, res) => {
  ...
  const workerProfile = await workerService.delete({ workerProfileId: req.userState.worker.id });
...
export const addWorkerGovernments = asyncHandler(async (req, res) => {
  ...
  const addedGovernmentsCount = await workerService.addWorkGovernments({ workerProfileId: req.userState.worker.id, governmentIds });

  new SuccessResponse("added worker working governments successfully", { addedGovernmentsCount }, 200).send(res);
});
...
export const deleteWorkerGovernments = asyncHandler(async (req, res) => {
  ...
  if (all) deletedGovernmentsCount = await workerService.deleteWorkGovernments({ workerProfileId: req.userState.worker.id, governmentIds });
  else if (governmentIds) deletedGovernmentsCount = await workerService.deleteWorkGovernments({ workerProfileId: req.userState.worker.id, governmentIds });

  new SuccessResponse("deleted worker working governments successfully", { deletedGovernmentsCount }, 200).send(res);
});
...
export const addWorkerSpecializations = asyncHandler(async (req, res) => {
  ...
  const addedSpecializationsCount = await workerService.addSpecializations({ workerProfileId: req.userState.worker.id, specializationsTree });

  new SuccessResponse("added worker specializations successfully", { addedSpecializationsCount }, 200).send(res);
});

/**
 */
export const deleteWorkerSpecializations = asyncHandler(async (req, res) => {
  const { mainSpecializationIds, specializationsTree, all } = matchedData(req, { includeOptionals: true });

  let deletedSpecializationsCount = 0;
  if (all) deletedSpecializationsCount = await workerService.deleteSpecializations({ workerProfileId: req.userState.worker.id, mainSpecializationIds: undefined });
  else {
    if (mainSpecializationIds)
      deletedSpecializationsCount = await workerService.deleteSpecializations({ workerProfileId: req.userState.worker.id, mainSpecializationIds });

    if (specializationsTree)
      deletedSpecializationsCount = await workerService.deleteSubSpecializations({ workerProfileId: req.userState.worker.id, specializationsTree });
  }

  new SuccessResponse("deleted worker specializations successfully", { deletedSpecializationsCount }, 200).send(res);
});
